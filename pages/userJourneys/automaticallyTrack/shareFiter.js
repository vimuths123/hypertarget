...
import {
      ...
      const filterIdGetTest=query.filterId?query.filterId:null
        if(filterIdGetTest==null){
          ...
          setWebsite(userJourneyResponse.website)
            ...
            const userJourneyResponse = await userJourneyService.getUserJourneysAutoFilterShare(id,decoded.startDate,decoded.endDate,decoded.filterUrls,invitationId2);
          if(userJourneyResponse.status==200){
          setUserJourneyArr(userJourneyResponse.userjourneys)
          setWebsite(userJourneyResponse.website)
          }
  ...
  const getDateRange= async (range)=>{
    ...
    if(isAddFilter){
      const userJourneyResponse=await userJourneyService.getUserJourneysAutoFilterShare( websiteId,startDate,endDate,filterUrls,invitationId)
      setUserJourneyArr(userJourneyResponse.userjourneys)
  ...
  {errorMsg === 'Request failed with status code 400' ? 'Something went wrong!' : (errorMsg=="this is not public!"? "Sorry, You Are Not Allowed to Access This Page":(errorMsg=="website not found!"?"invalid url":errorMsg))}
</div>
:
